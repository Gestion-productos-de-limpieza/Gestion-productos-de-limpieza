...
class UsuariosServices:
    ...
    def listar(self, rol: str = None) -> list[UsuarioListItem]:
        """Obtiene la lista de todos los usuarios, opcionalmente filtrada por rol."""
        usuarios = self.repo.obtener_por_rol(rol) if rol else self.repo.obtener_todos()
    ...
    def eliminar(self, id_a_eliminar: int, id_admin_activo: int) -> dict:
        ...
        if not self.repo.eliminar(id_a_eliminar):
            raise ValueError(f"El usuario con ID {id_a_eliminar} no existe")

        return {"mensaje": "Usuario eliminado exitosamente", "success": True}
